...
export const AIToolsTab: React.FC<AIToolsTabProps> = ({
  ...
  const handleGenerateSummary = useCallback(async () => {
    ...
    const archivedSummary = findArchivedItemAnalysis<string>(
      item.identifier,
  ...
  const handleExtractEntities = useCallback(async () => {
    ...
    const archivedEntities = findArchivedItemAnalysis<ExtractedEntities>(
      item.identifier,
      AIGenerationType.Entities,
      aiArchive,
    );
    if (archivedEntities) {
        ...
        <div className="flex flex-wrap gap-2">
          {items.map((e, i) => (
            <button
              ...
              className="bg-gray-700 hover:bg-accent-600 text-gray-200 hover:text-white text-xs font-medium px-2 py-1 rounded-full transition-colors"
            >
              {e}
            </button>
          ))}
  ...
  const ErrorDisplay: React.FC<{ error: string | null; onRetry: () => void }> = ({
    error,
    onRetry,
  }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center space-y-2 text-red-400 text-sm p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
        ...
        <button
          onClick={onRetry}
          className="px-3 py-1 bg-accent-600 text-white text-xs font-semibold rounded-lg hover:bg-accent-500 transition-colors"
        >
          {t('common:retry')}
        </button>
      </div>
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-semibold text-accent-400 flex items-center">
            <SparklesIcon className="w-5 h-5 mr-2" /> {t('aiTools:summaryTitle')}
          </h3>
          <button
            onClick={handleGenerateSummary}
            disabled={isSummarizing || !textContent}
            className="flex items-center space-x-2 px-3 py-1 text-sm font-medium rounded-lg transition-colors bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed"
          >
        ...
        <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-700 min-h-[100px]">
          ...
          {!isSummarizing && !summaryError && !summary && (
            <p className="text-gray-500 text-sm">{t('aiTools:generatePrompt')}</p>
          )}
        </div>
      </div>
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold text-accent-400 flex items-center">
            <TagIcon className="h-5 w-5 mr-2" /> {t('aiTools:entityAnalysisTitle')}
          </h3>
          <button
            onClick={handleExtractEntities}
            disabled={isExtracting || !textContent}
            className="flex items-center space-x-2 px-3 py-1 text-sm font-medium rounded-lg transition-colors bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed"
          >
        ...
        <div className="bg-gray-900/50 rounded-lg p-3 border border-gray-700 min-h-[100px] space-y-4">
          ...
          {!isExtracting && !entityError && !entities && (
            <p className="text-gray-500 text-sm">{t('aiTools:extractPrompt')}</p>
          )}
        </div>
      </div>
    </div>
