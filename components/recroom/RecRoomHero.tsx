...
export const RecRoomHero: React.FC = () => {
    ...
    const fetchFeatured = async () => {
      ...
      try {
        ...
        const data = await searchArchive(
          'collection:softwarelibrary_msdos_games AND avg_rating:[4.5 TO 5]',
          1,
          ['-random'],
          undefined,
          1,
        ...
        if (data.response?.docs.length > 0) {
          ...
          const fallbackData = await searchArchive(
  ...
  if (isLoading) {
    return (
      <div className="aspect-video bg-gray-800 rounded-xl flex items-center justify-center animate-pulse"></div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <p className="text-sm font-semibold text-accent-400 uppercase tracking-wider">
          {t('recRoom:heroFeatured')}
        </p>
