      ...
      {!isEditing && (
        <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity absolute right-1">
          <button
            ...
            aria-label={t('favorites:renameCollection')}
          >
          ...
          >
            <TrashIcon className="w-5 h-5 text-red-400/80" />
          </button>
        </div>
      )}
    </div>
...
export const LibrarySidebar: React.FC<LibrarySidebarProps> = ({
  ...
  const mediaTypeFilters: {
    ...
    {
      type: MediaType.Software,
      label: t('uploaderDetail:stats.software'),
      icon: <JoystickIcon className="w-5 h-5" />,
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <FilterButton
                ...
                isActive={filter.type === 'dashboard'}
                onClick={() => setFilter({ type: 'dashboard' })}
              />
              <FilterButton
                label={t('favorites:sidebar.allItems')}
