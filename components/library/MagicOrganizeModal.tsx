...
export const MagicOrganizeModal: React.FC<MagicOrganizeModalProps> = ({ itemIds, onClose }) => {
  ...
  useEffect(() => {
    ...
    organizeLibraryItems(
      itemsToOrganize.map((item) => ({ title: item.title, description: item.creator?.toString() })),
      language,
    )
      .then((res) => {
        setSuggestions(res);
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-gray-800 w-full max-w-lg rounded-xl shadow-2xl flex flex-col"
      ...
      >
        ...
        <footer className="px-6 py-4 bg-gray-900/50 rounded-b-xl text-right">
          <button
            ...
            className="bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-2 px-4 rounded-lg transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed"
          >
            {t('common:confirm')}
          </button>
        </footer>
