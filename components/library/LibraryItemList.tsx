...
const BulkActionsToolbar: React.FC<{ selectedIds: string[]; onClear: () => void }> = ({
  selectedIds,
  onClear,
}) => {
  const { t } = useLanguage();
  ...
  return (
    <div className="flex-shrink-0 p-2 bg-gray-700/50 rounded-lg flex items-center justify-between animate-fade-in">
      <span className="text-sm font-semibold text-white pl-2">
        {t('favorites:bulkActions.selected', { count })}
      </span>
      <div className="flex items-center gap-1">
        <button
          onClick={() => setModal({ type: 'addToCollection', itemIds: selectedIds })}
          title={t('favorites:bulkActions.addToCollection')}
          className="p-2 rounded hover:bg-gray-600"
        >
          <CollectionIcon className="w-5 h-5" />
        </button>
        <button
          onClick={() => setModal({ type: 'addTags', itemIds: selectedIds })}
          title={t('favorites:bulkActions.addTags')}
          className="p-2 rounded hover:bg-gray-600"
        >
          <TagIcon className="w-5 h-5" />
        </button>
        <button
          onClick={() => setModal({ type: 'magicOrganize', itemIds: selectedIds })}
          title={t('favorites:bulkActions.magicOrganize')}
          className="p-2 rounded hover:bg-gray-600"
        >
