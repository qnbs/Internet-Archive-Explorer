...
const NavButton: React.FC<{ item: NavItem; isActive: boolean; onClick: (view: View) => void }> = ({
  item,
  isActive,
  onClick,
}) => {
...
export const SideMenu: React.FC<SideMenuProps> = ({
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-800 shadow-lg z-40 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:shadow-none flex flex-col border-r border-gray-200 dark:border-gray-700/50`}
      >
        {/* Desktop Logo Header */}
        ...
        <div className="flex-grow flex flex-col overflow-y-auto">
          <div className="p-4">
            ...
            <nav>
              ...
              <hr className="my-4 border-gray-200 dark:border-gray-700/50" />
              <div>{utilityNavItems.map(renderNavItem)}</div>
            </nav>
          </div>
        </div>
      </aside>
