...
export const AIArchiveItemCard: React.FC<AIArchiveItemCardProps> = React.memo(
  ({ entry, isSelected, onSelect }) => {
    const { t, language } = useLanguage();
    const [imageError, setImageError] = useState(false);
    const date = new Date(entry.timestamp).toLocaleDateString(language, {
      month: 'short',
      day: 'numeric',
      year: 'numeric',
    });
    ...
    return (
      ...
      >
        <div
          className={`flex-shrink-0 mt-1 w-10 h-10 rounded-md flex items-center justify-center text-cyan-300 ${isSelected ? 'bg-cyan-500/20' : 'bg-gray-700'}`}
        >
        ...
        <div className="flex-grow min-w-0">
          <div className="flex justify-between items-start">
            <h3
              className={`font-semibold text-sm truncate pr-2 ${isSelected ? 'text-white' : 'text-gray-200'}`}
          ...
          <p className="text-xs text-gray-400 truncate italic line-clamp-2">
            "{getContentSnippet(entry)}"
          </p>
        </div>
      </article>
    );
