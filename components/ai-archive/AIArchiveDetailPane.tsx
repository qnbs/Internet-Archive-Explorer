...
const Tag: React.FC<{ label: string; onRemove: () => void }> = ({ label, onRemove }) => (
  <div className="flex items-center gap-1.5 bg-cyan-500/80 text-white text-xs font-semibold pl-2 pr-1 py-1 rounded-full">
    <span>{label}</span>
    <button onClick={onRemove} className="rounded-full hover:bg-black/20">
    ...
    return (
      <div className="space-y-3 text-sm">
        {entities.people.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-400 mb-1">{t('common:people')}</h4>
            <p>{entities.people.join(', ')}</p>
          </div>
        )}
        {entities.places.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-400 mb-1">{t('common:places')}</h4>
            <p>{entities.places.join(', ')}</p>
          </div>
        )}
        {entities.organizations.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-400 mb-1">{t('common:organizations')}</h4>
            <p>{entities.organizations.join(', ')}</p>
          </div>
        )}
        {entities.dates.length > 0 && (
          <div>
            <h4 className="font-semibold text-gray-400 mb-1">{t('common:dates')}</h4>
            <p>{entities.dates.join(', ')}</p>
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="space-y-3 text-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {analysis.tags.map((t) => (
            <span key={t} className="bg-gray-700 text-cyan-300 px-2 py-1 rounded-full text-xs">
              {t}
            </span>
          ))}
        </div>
      ...
      <div className="space-y-3 text-sm">
        <div>
          <h4 className="font-semibold text-gray-400 mb-1">
            {t('favorites:magicOrganize.suggestedTags')}
          </h4>
          <p>{suggestions.tags.join(', ')}</p>
        </div>
        <div>
          <h4 className="font-semibold text-gray-400 mb-1">
            {t('favorites:magicOrganize.suggestedCollections')}
          </h4>
          <p>{suggestions.collections.join(', ')}</p>
        </div>
      </div>
    );
  }
...
export const AIArchiveDetailPane: React.FC<{
  ...
  const date = new Date(selectedEntry.timestamp).toLocaleString(language, {
    dateStyle: 'long',
    timeStyle: 'short',
  });
      ...
      <header className="flex-shrink-0 pb-3 flex items-center justify-between gap-3 border-b border-gray-700">
        ...
        <div className="min-w-0">
          <h2 className="text-md font-bold text-white truncate">
            {t(`aiArchive:types.${selectedEntry.type}`)}
          </h2>
          <p className="text-sm text-gray-400 truncate">{date}</p>
        </div>
        <button
          onClick={handleDelete}
          className="ml-auto p-2 text-gray-400 hover:text-red-400 rounded-full"
        >
      ...
      <div className="flex-grow overflow-y-auto mt-4 space-y-4 pr-1">
          ...
          <div className="mt-1">
            ...
            <div className="space-y-2">
              {selectedEntry.sources.map((s) => (
                <SourceItemCard key={s.identifier} source={s} />
              ))}
            </div>
          </div>
        )}
        ...
        <div>
          <h3 className="font-semibold text-gray-300 mb-2 flex items-center gap-2">
            <TagIcon className="w-5 h-5 text-gray-400" /> {t('favorites:details.tags')}
          </h3>
          <div className="flex flex-wrap gap-2">
