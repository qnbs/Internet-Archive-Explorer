...
export const AIArchiveDetailModal: React.FC<AIArchiveDetailModalProps> = ({ entry, onClose }) => {
  ...
  const date = new Date(entry.timestamp).toLocaleString(language, {
    dateStyle: 'long',
    timeStyle: 'short',
  });
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-gray-800 w-full max-w-2xl max-h-[85vh] rounded-xl shadow-2xl flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center justify-between p-4 border-b border-gray-700 flex-shrink-0">
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-white truncate">
              {t(`aiArchive:types.${entry.type}`)}
            </h2>
