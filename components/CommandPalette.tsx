...
const SectionIcon: React.FC<{ section: string }> = ({ section }) => {
  ...
  if (section.toLowerCase().includes('ai'))
    return <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v10a2 2 0 01-2 2h-2" /></svg>;
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ onClose }) => {
  ...
  const commandActions = {
    navigateTo: (view: View) => { setActiveView(view); onClose(); },
    globalSearch: (q: string) => { setGlobalSearchQuery(q); setActiveView('explore'); onClose(); },
  ...
  const extraCommands = [
    {
      ...
      action: () => { setActiveView('aiArchive'); onClose(); },
    ...
    },
    {
      ...
      action: () => {
        navigator.clipboard.writeText(window.location.href).then(() => addToast(t('commandPalette:shareCopied') || 'Link copied!', 'success'));
        onClose();
      },
    ...
    },
    {
      ...
      action: () => { setActiveView('library'); onClose(); },
        ...
        >
          ...
          >
            <div className="flex items-center px-4 py-3 border-b border-gray-200 dark:border-gray-700">
              ...
              />
              <kbd className="hidden md:inline-flex items-center px-1.5 py-0.5 rounded text-xs text-gray-400 border border-gray-300 dark:border-gray-600 ml-2 shrink-0">ESC</kbd>
            </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-2 flex items-center justify-between text-xs text-gray-400">
              <span><kbd className="px-1.5 py-0.5 rounded border border-gray-300 dark:border-gray-600 mr-1">↑↓</kbd>{t('commandPalette:navigate') || 'Navigate'}</span>
              <span><kbd className="px-1.5 py-0.5 rounded border border-gray-300 dark:border-gray-600 mr-1">↵</kbd>{t('commandPalette:select') || 'Select'}</span>
              <span><kbd className="px-1.5 py-0.5 rounded border border-gray-300 dark:border-gray-600 mr-1">ESC</kbd>{t('commandPalette:close') || 'Close'}</span>
            </div>
          </Command>
        </motion.div>
      </motion.div>
