...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-start pt-24 p-4 transition-opacity duration-300 ${isMounted ? 'opacity-100' : 'opacity-0'}`}
      onClick={onClose}
    >
      <div
        ref={paletteRef}
        ...
        <div id="command-list" role="listbox" className="max-h-[60vh] overflow-y-auto p-2">
          {flatCommands.length > 0 ? (
            Object.entries(groupedCommands).map(
              ([section, commandsInSection]: [string, Command[]]) => (
