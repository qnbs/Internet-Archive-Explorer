...
const MetadataRow: React.FC<{ label: string; children: React.ReactNode; isButton?: boolean }> =
  React.memo(({ label, children, isButton }) => (
    <div className="flex justify-between items-start text-sm">
      <span className="font-semibold text-gray-500 dark:text-gray-400 flex-shrink-0 mr-2">
        {label}:
      </span>
      {isButton ? (
        ...
        <span
          className="text-gray-800 dark:text-gray-200 text-right truncate"
...
export const ItemDetailSidebar: React.FC<ItemDetailSidebarProps> = ({
  ...
  const formatDate = (dateStr: string) =>
    new Date(dateStr).toLocaleDateString(language, {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
    ...
    <div className="space-y-4">
        ...
        {playableMedia?.type === 'audio' && isPlaying && (
          <div className="absolute bottom-0 left-0 right-0 p-2 bg-black/60 backdrop-blur-sm pointer-events-none">
            <div className="flex items-center text-white animate-fade-in">
              ...
              <span className="text-sm font-semibold truncate flex-grow" title={item.title}>
                {item.title}
              </span>
            </div>
          </div>
      ...
      {metadata.metadata['access-restricted-item'] === 'true' && (
        <div className="p-3 bg-yellow-500/10 text-yellow-300 rounded-lg border border-yellow-500/30 flex items-start space-x-2 text-sm">
          ...
          <div>
            <span className="font-bold">{t('modals:details.restrictedAccess')}</span>:{' '}
            {t('modals:details.restrictedAccessDesc')}
          </div>
        </div>
      )}
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700 space-y-2">
            ...
            <div className="flex flex-col items-end">
              {creators.map((c, i) => (
                <button
                  ...
                  className="text-cyan-600 dark:text-cyan-400 hover:underline ml-1 truncate"
                  title={t('modals:details.searchByCreator', { creator: c })}
                >
                  {c}
                </button>
              ))}
        ...
        {uploader && (
          <MetadataRow label={t('common:uploader')} isButton>
            <button
              ...
              title={t('modals:details.viewUploaderProfile', {
                uploader: formatIdentifierForDisplay(uploader),
              })}
            >
        ...
        <MetadataRow label={t('common:published')}>{formatDate(item.publicdate)}</MetadataRow>
        {metadata.metadata.licenseurl && (
          <MetadataRow label={t('common:license')}>
            <a
              href={metadata.metadata.licenseurl}
              target="_blank"
