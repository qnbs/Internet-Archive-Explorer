...
const ViewerToolbar: React.FC<{
  ...
  const controls = [
    ...
    {
      label: t('common:imageViewer.rotateRight'),
      action: viewer.rotateCW,
      icon: <RotateClockwiseIcon />,
    },
    {
      label: t('common:imageViewer.rotateLeft'),
      action: viewer.rotateCCW,
    ...
    {
      label: t('common:imageViewer.download'),
      action: onDownload,
    ...
    <div className="space-y-4">
          ...
          <div className="space-y-3 text-sm animate-fade-in">
            ...
            {initialAnalysis.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {initialAnalysis.tags.map((tag) => (
                  <TagButton key={tag} tag={tag} onCloseModal={onCloseModal} />
                ))}
              </div>
            )}
          </div>
      ...
      <form onSubmit={handleAsk} className="flex gap-2 items-center">
        ...
        <button
          ...
          className="p-2 bg-cyan-600 rounded-lg text-white disabled:bg-gray-500"
        >
...
const ImageDetailModalContent: React.FC<Omit<ImageDetailModalProps, 'item'>> = ({
  onClose,
  onCreatorSelect,
  onUploaderSelect,
}) => {
  ...
  useEffect(() => {
    ...
    const archivedAnalysis = findArchivedItemAnalysis<ImageAnalysisResult>(
      item.identifier,
      AIGenerationType.ImageAnalysis,
      aiArchive,
    );
    if (archivedAnalysis) {
        ...
        >
          {imageUrl && !imageError ? (
            <img
              src={imageUrl}
              alt={item.title}
              onError={() => setImageError(t('common:imageViewer.loadingError'))}
        ...
        <aside className="w-full md:w-80 lg:w-96 flex-shrink-0 bg-gray-800/50 flex flex-col">
          <header className="flex items-center justify-between p-4 border-b border-gray-700">
            <div className="flex items-center space-x-3 min-w-0">
              ...
              <h2 id="modal-title" className="text-lg font-bold text-white truncate">
                {item.title}
              </h2>
            </div>
            <button
                  ...
                  <div className="border border-gray-700 rounded-lg">
                    ...
                    >
                      <h3 className="font-semibold text-cyan-400 flex items-center gap-2">
                        <SparklesIcon /> {t('common:aiAnalysis')}
                      </h3>
