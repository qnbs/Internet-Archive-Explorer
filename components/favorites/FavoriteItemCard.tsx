...
export const FavoriteItemCard: React.FC<LibraryItemCardProps> = React.memo(
  ({ item, isSelected, onSelect, isSelectMode, isDetailViewTarget }) => {
    const { t } = useLanguage();
      ...
      >
        ...
        <div className="flex-grow min-w-0">
          <div className="flex items-center gap-2">
            <h3
              className={`font-semibold text-sm truncate ${isDetailViewTarget ? 'text-white' : 'text-gray-200'}`}
              title={item.title}
            >
              {item.title}
            </h3>
            {isRestricted && (
              <div className="flex-shrink-0" title={t('modals:details.restrictedItemTooltip')}>
                <InfoIcon className="w-4 h-4 text-yellow-400" />
              </div>
            )}
          </div>
          <p className="text-xs text-gray-400 truncate" title={creatorName}>
            {creatorName}
          </p>
