...
const StatusIcon: React.FC<{ status: DownloadItem['status'] }> = ({ status }) => {
  if (status === 'done')
    return <svg className="w-4 h-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>;
  if (status === 'error')
    return <svg className="w-4 h-4 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>;
  if (status === 'downloading')
    return <svg className="w-4 h-4 text-accent-400 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>;
  return <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
};
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white text-sm">
                {t('downloads:title') || 'Downloads'}
                {activeCount > 0 && (
                  <span className="ml-2 bg-accent-500/15 text-accent-600 dark:text-accent-400 text-xs px-1.5 py-0.5 rounded-full">{activeCount} aktiv</span>
                )}
              </h3>
                ...
                queue.slice().reverse().map((item) => (
                  <div key={item.id} className="p-3 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(item.status === 'downloading' || item.status === 'queued') && item.sizeBytes && (
                        <div className="mt-1.5">
                          ...
                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                            <motion.div
                              ...
                              initial={{ width: 0 }}
                              animate={{ width: `${(item.downloadedBytes / item.sizeBytes) * 100}%` }}
                      ...
                      {item.status === 'error' && (
                        <p className="text-xs text-red-500 mt-0.5">{t('downloads:failed') || 'Download failed'}</p>
                      )}
                    </div>
                    <button
