...
export const ReviewCard: React.FC<ReviewCardProps> = ({ review }) => {
  ...
  const reviewDate = review.reviewdate
    ? new Date(review.reviewdate).toLocaleDateString(language, {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
      })
  ...
  return (
    <article className="bg-gray-800/60 p-4 rounded-lg border border-gray-700/50">
      ...
      {review.reviewtitle && (
        <h3 className="font-bold text-white text-md mb-1">{review.reviewtitle}</h3>
      )}
      {/* FIX: Access reviewbody property. */}
      {review.reviewbody && (
        <blockquote className="text-gray-300 text-sm border-l-4 border-cyan-500 pl-4 italic my-2">
      ...
      <div className="text-xs text-gray-400 mt-3">
        ...
        <p className="mt-1">
          ...
          <a href="#" onClick={handleItemClick} className="text-cyan-400 hover:underline">
            {review.title}
          </a>
        </p>
      </div>
