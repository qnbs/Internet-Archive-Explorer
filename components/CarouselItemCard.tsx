...
export const CarouselItemCard: React.FC<CarouselItemCardProps> = React.memo(
  ({ item, aspectRatio, index }) => {
    ...
    return (
      ...
      >
        <div
          className={`relative bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-accent-500/10 dark:hover:shadow-accent-500/30 transition-shadow duration-300 ${aspectClasses[aspectRatio]}`}
        >
          {isRestricted && (
            <div
        ...
        <div className="pt-2 px-1">
          <h3
            className="text-sm font-semibold text-gray-800 dark:text-white group-hover:text-accent-600 dark:group-hover:text-accent-400 transition-colors h-10 line-clamp-2"
            title={item.title}
          >
            {item.title}
          </h3>
