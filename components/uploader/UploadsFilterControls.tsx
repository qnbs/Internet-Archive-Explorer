      ...
      [
        {
          ...
          label: t('uploaderDetail:filters.allTypes'),
        },
        {
          key: 'movies',
          type: 'movies',
          icon: <MovieIcon className="w-4 h-4" />,
          label: t('uploaderDetail:stats.movies'),
        },
        {
          key: 'audio',
          type: 'audio',
          icon: <AudioIcon className="w-4 h-4" />,
          label: t('uploaderDetail:stats.audio'),
        },
        {
          key: 'texts',
          type: 'texts',
          icon: <BookIcon className="w-4 h-4" />,
          label: t('uploaderDetail:stats.texts'),
        },
        {
          key: 'image',
          type: 'image',
          icon: <ImageIcon className="w-4 h-4" />,
          label: t('uploaderDetail:stats.images'),
        },
        {
          key: 'software',
          type: 'software',
          icon: <JoystickIcon className="w-4 h-4" />,
          label: t('uploaderDetail:stats.software'),
        },
      ] as {
        key: keyof UploaderStats;
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      <div className="flex items-center flex-wrap gap-2">
        <span className="text-sm text-gray-500 dark:text-gray-400 font-medium mr-2 shrink-0">
          {t('uploaderDetail:filters.title')}:
        </span>
        {statItems.map((stat) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full ${activeFilter === stat.type ? 'bg-white/20' : 'bg-gray-200 dark:bg-gray-800/60'}`}
              >
      ...
      <div className="flex items-center justify-start lg:justify-end gap-2 shrink-0">
        ...
        <button
          ...
          aria-label={t('uploaderDetail:toggleSort', {
            direction:
              sortDirection === 'asc'
                ? t('uploaderDetail:ascending')
                : t('uploaderDetail:descending'),
          })}
        >
          {sortDirection === 'asc' ? (
