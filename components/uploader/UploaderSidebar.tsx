...
import { ArrowLeftIcon, ExternalLinkIcon, StarIcon } from '@/components/Icons';
...
const StatDisplay: React.FC<{ label: string; value: number; isLoading: boolean }> = ({
  label,
  value,
  isLoading,
}) => (
...
export const UploaderSidebar: React.FC<UploaderSidebarProps> = ({
  profile,
  stats,
  isLoadingStats,
  onBack,
  ...
  const getUploaderUrl = () => {
    const isStandardProfile =
      !profile.curatedData?.searchField ||
      ['uploader', 'creator'].includes(profile.curatedData.searchField);
  ...
  const backOrDisconnectLabel = onDisconnect
    ? t('myArchive:dashboard.disconnect')
    : t('uploaderDetail:back');
  ...
  return (
    <div className="bg-white dark:bg-gray-800/60 p-5 rounded-xl border border-gray-200 dark:border-gray-700/50 space-y-4 sticky top-20">
      {handleBackOrDisconnect && (
        <button
          onClick={handleBackOrDisconnect}
          className="flex items-center space-x-2 text-sm text-accent-600 dark:text-accent-400 hover:underline mb-2"
        >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex items-center flex-shrink-0">
          ...
          <a
            ...
            className="p-2 text-gray-400 hover:text-accent-400 transition-colors rounded-full"
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
        <h3 className="font-semibold text-gray-900 dark:text-white mb-3">
          {t('uploaderDetail:stats.distribution')}
        </h3>
        {isLoadingStats ? (
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">
          {t('myArchive:dashboard.statsTitle')}
        </h3>
        <StatDisplay
          label={t('uploaderDetail:stats.uploads')}
          value={stats?.total || 0}
          isLoading={isLoadingStats}
        />
        <StatDisplay
          label={t('uploaderDetail:stats.collections')}
          value={stats?.collections || 0}
          isLoading={isLoadingStats}
        />
        <StatDisplay
          label={t('uploaderDetail:stats.favorites')}
          value={stats?.favorites || 0}
          isLoading={isLoadingStats}
        />
        <StatDisplay
          label={t('uploaderDetail:stats.reviews')}
