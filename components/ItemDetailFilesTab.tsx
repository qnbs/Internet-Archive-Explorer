...
export const ItemDetailFilesTab: React.FC<ItemDetailFilesTabProps> = ({
  files,
  itemIdentifier,
}) => {
      ...
      {filteredFiles.map((file: ArchiveFile) => (
        ...
        >
          <div className="flex items-center space-x-3 truncate">
            <FileIcon />
            <span className="truncate text-gray-800 dark:text-gray-300 text-sm" title={file.name}>
              {file.name}
            </span>
          </div>
          <div className="flex items-center space-x-4 flex-shrink-0 ml-4">
            ...
            <span className="text-xs font-mono bg-gray-100 text-cyan-800 dark:bg-gray-600 dark:text-cyan-300 px-2 py-0.5 rounded">
              {file.format}
            </span>
          </div>
        </a>
