...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({
  ...
  return (
    <div
      className={`h-full flex flex-col bg-gray-900/50 rounded-lg border border-gray-700 ${className || ''}`}
    >
      <div className="flex-shrink-0 flex items-center space-x-1 p-1 border-b border-gray-700">
        <ToolbarButton onClick={(e) => handleCommand(e, 'bold')} title={t('common:richText.bold')}>
          <BoldIcon />
        </ToolbarButton>
        <ToolbarButton
        ...
        <ToolbarButton
          ...
          title={t('common:richText.unorderedList')}
        >
          <ListUnorderedIcon />
        </ToolbarButton>
        <ToolbarButton
          onClick={(e) => handleCommand(e, 'insertOrderedList')}
          title={t('common:richText.orderedList')}
        >
          <ListOrderedIcon />
        </ToolbarButton>
      </div>
