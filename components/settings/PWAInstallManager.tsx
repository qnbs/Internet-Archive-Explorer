...
export const PWAInstallManager: React.FC = () => {
  ...
  const handleInstallClick = () => {
    if (deferredPrompt) {
      // Browser-native install prompt is available.
      setModal({ type: 'pwaInstall' });
      return;
    }

    if (isIOS) {
      // iOS has no native prompt; show the manual instructions.
      setModal({ type: 'pwaInstall' });
      return;
    }

    // On desktop Chrome/Edge/Samsung the prompt may have been suppressed by
    // the browser. Show the modal anyway; it explains the browser-menu fallback.
    setModal({ type: 'pwaInstall' });
  };
    ...
    return (
      <div>
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white pt-2">
          {t('settings:data.installApp')}
        </h3>
        <div className="py-4 border-b border-gray-200 dark:border-gray-700">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            {t('settings:data.installAppDesc')}
          </p>
