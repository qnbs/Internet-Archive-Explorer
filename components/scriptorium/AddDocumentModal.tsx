...
export const AddDocumentModal: React.FC<AddDocumentModalProps> = ({ workset, onAdd, onClose }) => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    searchArchive(`(${debouncedQuery}) AND mediatype:(texts)`, 1, [], undefined, 10)
      .then((data) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-start pt-20 p-4"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={modalRef}
        className="bg-gray-800 w-full max-w-2xl rounded-xl shadow-2xl flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center justify-between p-4 border-b border-gray-700">
          <h2 id="add-doc-title" className="text-lg font-bold text-white">
            {t('scriptorium.addDocumentTo', { worksetName: workset.name })}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4">
          <div className="relative">
            <label htmlFor="doc-search-input" className="sr-only">
              {t('scriptorium.searchPlaceholder')}
            </label>
        ...
        <div className="p-4 min-h-[300px] max-h-[60vh] overflow-y-auto">
          ...
          {!isLoading && results.length === 0 && (
            <p className="text-center text-gray-500 pt-8">
              {debouncedQuery ? t('common:noResultsFound') : t('scriptorium.startTyping')}
            </p>
          )}
            ...
            {results.map((item) => (
              ...
              >
                ...
                <button
                  ...
                  className="flex-shrink-0 flex items-center space-x-2 text-sm bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-1 px-3 rounded-md transition-colors"
                >
