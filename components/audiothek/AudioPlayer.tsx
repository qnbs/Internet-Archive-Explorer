      ...
      <div className="bg-gray-800/80 backdrop-blur-md border-t border-gray-700/50 p-3 shadow-t-lg flex items-center gap-4">
        ...
        <div className="flex-grow min-w-0">
          ...
          <p className="text-sm text-gray-400 truncate">
            {Array.isArray(currentTrack.creator)
              ? currentTrack.creator.join(', ')
              : currentTrack.creator}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => prev()} className="p-1 text-gray-300 hover:text-white">
            <ChevronLeftIcon />
          </button>
          <button
            onClick={() => togglePlayPause()}
            className="w-10 h-10 flex items-center justify-center bg-cyan-500 text-white rounded-full hover:bg-cyan-400 transition-colors"
          >
            {isPlaying ? <PauseIcon className="w-6 h-6" /> : <PlayIcon className="w-6 h-6" />}
          </button>
          <button onClick={() => next()} className="p-1 text-gray-300 hover:text-white">
            <ChevronRightIcon />
          </button>
        </div>
        ...
        <div className="flex items-center">
          <button
            ...
            className={`p-2 rounded-lg ${isPlaylistOpen ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-400 hover:bg-gray-700'}`}
          >
