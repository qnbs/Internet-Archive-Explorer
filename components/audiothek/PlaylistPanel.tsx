...
export const PlaylistPanel: React.FC<PlaylistPanelProps> = ({ onClose }) => {
  ...
  return (
    <div
      ref={panelRef}
      className="absolute bottom-full right-4 mb-2 w-80 bg-gray-900/80 backdrop-blur-lg border border-gray-700 rounded-xl shadow-2xl flex flex-col max-h-[50vh] animate-fade-in-left"
    >
      <header className="flex justify-between items-center p-3 border-b border-gray-700 flex-shrink-0">
        ...
        <div>
          <button
            ...
            className="p-1.5 text-gray-400 hover:text-red-400 rounded-md text-xs font-semibold mr-2"
          >
            {t('audiothek:player.clearPlaylist')}
          </button>
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-white rounded-md">
          ...
          playlist.map((track, index) => (
            ...
            >
              ...
              <div className="flex-grow min-w-0">
                <p
                  className={`text-sm font-semibold truncate ${index === currentIndex ? 'text-cyan-300' : 'text-white'}`}
                >
                  {track.title}
                </p>
                <p className="text-xs text-gray-400 truncate">
                  {Array.isArray(track.creator) ? track.creator.join(', ') : track.creator}
                </p>
              </div>
