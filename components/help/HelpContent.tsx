  ...
  return (
    <div className="border-b border-gray-700">
      ...
      >
        ...
        />
        <svg
          className={`w-6 h-6 text-accent-400 transform transition-transform ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      >
        <div className="overflow-hidden">
          <div
            className="p-4 pt-0 text-gray-300 leading-relaxed space-y-3 prose-a:text-accent-400 hover:prose-a:underline prose-ul:list-disc prose-ul:pl-6 prose-li:mb-1 prose-code:bg-gray-700 prose-code:px-1 prose-code:rounded prose-strong:text-gray-200"
...
export const HelpContent: React.FC = () => {
  ...
  const filteredQuestions = React.useMemo(() => {
    ...
    return activeTopic.questions.filter(
      (qa) => qa.q.toLowerCase().includes(lowerQuery) || qa.a.toLowerCase().includes(lowerQuery),
    );
