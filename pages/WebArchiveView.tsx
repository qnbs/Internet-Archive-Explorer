      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white dark:bg-gray-800/60 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700/50 min-h-[300px]">
          {isLoading ? (
            ...
            <div className="flex justify-center items-center h-full text-center text-gray-400 px-4">
              <p>{error}</p>
            </div>
                ...
                <thead className="text-xs text-gray-400 uppercase bg-gray-700/50 sticky top-0">
                  <tr>
                    <th scope="col" className="px-6 py-3">
                      {t('webArchive:resultsTable.snapshotDate')}
                    </th>
                    <th scope="col" className="px-6 py-3">
                      {t('webArchive:resultsTable.originalUrl')}
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={i} className="border-b border-gray-700 hover:bg-gray-700/50">
                        <td className="px-6 py-4 font-mono text-accent-400">
                          <a
                            href={`https://web.archive.org/web/${timestamp}/${original}`}
                            target="_blank"
                        ...
                        <td className="px-6 py-4 text-gray-300 truncate max-w-xs" title={original}>
                          {original}
                        </td>
                      </tr>
                    );
          ...
          <div className="space-y-2">
            {history.map((h) => (
              <button
                ...
                className="w-full text-left text-sm text-accent-400 hover:underline p-2 bg-gray-800/60 rounded-md truncate"
              >
                {h}
              </button>
            ))}
