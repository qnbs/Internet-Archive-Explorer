...
const getShelves = (t: (key: string) => string) => [
  ...
  {
    key: 'grateful_dead',
    title: t('audiothek:shelves.grateful_dead'),
...
const getAdditionalAudioCollections = (t: (key: string) => string) => [
  {
    key: 'archiveOfContemporaryMusic',
    title: t('audiothek:collections.archiveOfContemporaryMusic'),
    query: 'collection:archive_of_contemporary_music',
  },
  {
    key: 'hipHopMixtapes',
    title: t('audiothek:collections.hipHopMixtapes'),
    query: 'collection:hiphopmixtapes',
  },
  {
    key: 'radioWithASR',
    title: t('audiothek:collections.radioWithASR'),
    query: 'collection:radioprograms',
  },
  {
    key: 'blackieBeats',
    title: t('audiothek:collections.blackieBeats'),
    query: 'collection:blackie-beats-archive',
  },
...
const AudiothekView: React.FC = () => {
  ...
  return (
    <div
      ...
      role="region"
      aria-label={t('sideMenu:audiothek')}
    >
      <AudiothekHero />
      <CategoryGrid />
      ...
      {shelves.map((shelf) => (
        <AudioCarousel key={shelf.key} title={shelf.title} query={shelf.query} />
      ))}
      <div className="border-t border-gray-700"></div>
      {additionalAudioCollections.map((collection) => (
        <AudioCarousel key={collection.key} title={collection.title} query={collection.query} />
      ))}
    </div>
  );
};
